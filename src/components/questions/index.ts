...
export const getRandomQuestion = (difficult: QuestionDifficult, passQuestions?: number[]): TgetQuestion => {
  const questionsCollection = questions
    .filter(question => question.difficult === difficult && !passQuestions?.includes(question.id))
